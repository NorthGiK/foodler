...
- Добавлен отдельный редакционный лендинг Foodler для Android: он объясняет
  сканирование чеков, категории расходов и информационные рекомендации
  Foodler AI без медицинских обещаний.
- CTA скачивания на лендинге теперь предлагает установить приложение напрямую
  из APK или открыть его страницу в RuStore.
- В footer лендинга добавлена отдельная editorial-иконка Foodler в палитре
  лендинга; мобильная иконка приложения остаётся без изменений.

- AI-категоризация больше не теряет весь чек из-за длинных ключей или частичного
  ответа: отсутствующие строки и transient `429/5xx` повторяются один раз.
  Восстановлены структурированные runtime-логи безопасного состояния AI-конфига.

- Категоризация чеков стала server-canonical: каскад confirmed cache, точного
  GTIN, единственного alias/имени, локального правила и одного AI batch сохраняет
  snapshot категории; конфликтующие совпадения каталога передаются AI.

### fixed
- Исправлены очевидные ошибки категоризации фруктов и кормов для животных;
  итоговая сумма весовой позиции теперь восстанавливается как количество,
  умноженное на цену, если провайдер не передал `sum`.
- Для неоднозначной категоризации товаров используется `AI_STRONG_MODEL`.
- После строгого GTIN/alias/name lookup однозначное локальное правило завершает
  категоризацию, а неизвестные и конфликтующие позиции идут одним batch-вызовом
  `AI_STRONG_MODEL`; fuzzy сохранён только для поиска каталога.
- Рекомендации «Как сэкономить» получают серверную сводку трат перед запросом к
  AI; дополнительные данные теперь предлагаются только после основного ответа.
- AI-контекст отправляется без локального усечения, а кэш зависит от набора
  чеков и обновляется при повторном использовании ответа.

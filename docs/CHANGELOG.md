...
- Категоризация чеков стала server-canonical: batch cache/правила/AI сохраняют
  snapshot категории, а Open Food Facts используется лишь для кратких GTIN hints.
...
- После строгого GTIN/alias/name lookup локальные правила и Open Food Facts
  формируют только candidates для одного batch-вызова `AI_STRONG_MODEL`;
  fuzzy сохранён только для поиска каталога.

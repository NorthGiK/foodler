...
- AI-контекст больше не обрезается перед отправкой провайдеру. Запрос данных для
  будущих рекомендаций разрешён только после полноценного ответа.
- Локальные правила категорий теперь распознают фрукты вроде нектарина и
...
- Общий анализ теперь формирует единый ответ без служебных `action`-меток,
  учитывает направления экономии и состава корзины при списании 2 credits и

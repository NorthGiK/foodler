...
from src.ai_service import _build_user_prompt
...
def test_create_prompt_requests_markdown_and_renders_markers():
    ...
    assert "Эта фраза не" in prompt
    assert "заменяет ответ и рекомендации" in prompt
...
def test_overall_analysis_prompt_has_no_service_action_blocks():
    ...
    assert "до 1500 символов" in prompt
...
def test_build_user_prompt_does_not_truncate_ai_context():
    marker = "последний_товар_без_обрезания"
    prompt = _build_user_prompt("save-money", None, {"receipts": marker * 5000})

    assert marker * 5000 in prompt

...
чека сервер применяет каскад: подтверждённый cache, точный global GTIN,
единственный точный нормализованный alias/имя, однозначное локальное правило,
затем один AI batch для оставшихся позиций и безопасное `прочее`. Локальные
решения сохраняются с источником `local`; конфликтующие записи каталога не
выбираются произвольно и передаются в AI batch. Ответ модели запрашивается JSON
prompt'ом и допускает fenced JSON; timeout берётся из `AI_TIMEOUT_SECONDS`.
Недоступность AI, HTTP-ошибка или некорректный ответ не отменяют сохранение
чека: snapshot получает `прочее`. Confidence не ниже 0.8 сохраняется в
...
Сброс пароля выполняется в два шага: `confirm-code` подтверждает email-код и
выдаёт подписанный reset-токен на 10 минут, затем `reset` устанавливает новый
пароль. Код и reset-токен не хранятся в открытом виде.

...
каноническому продукту и GTIN, затем по точному нормализованному alias или имени.
Если совпадения нет, авторизованный запрос использует структурированный
AI-классификатор через `AI_STRONG_MODEL`; уверенный результат сохраняется как
канонический продукт вместе с alias. Анонимный запрос возвращает
`matched_by=none`. Fuzzy-поиск используется только отдельным
`GET /products/search` для поиска по каталогу и не участвует в категоризации.
Результаты с уверенностью ниже 0.8 не добавляются в каталог.

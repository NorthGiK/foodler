...
одну из допустимых товарных категорий через `AI_STRONG_MODEL`. Категории в API и
...
недоступен или возвращает некорректную категорию, новый продукт не сохраняется.
Сумма позиции в API рассчитывается как `quantity × price`.
...
товарные позиции проверенными `gtin` и `category`. Для уникальных позиций одного
чека сервер сначала использует подтверждённый cache (global GTIN, merchant-scoped
restricted code или нормализованное имя), затем локальные правила и один
structured batch-запрос. Локальные правила и каталог формируют только candidates,
а финальную категорию неизвестной позиции выбирает модель. Unknown global GTIN может получить только минимальные
category hints из Open Food Facts v3 (`product_type=all`, 3-second timeout);
raw catalogue responses не сохраняются. Коды 20–29 не отправляются туда и не
кэшируются глобально. Confidence не ниже 0.8 сохраняется в assignment cache;
nutrition product при сканировании не создаётся. Недоступность AI или каталога
не отменяет чек: snapshot получает безопасное `прочее`. Fuzzy-поиск используется
только отдельным `GET /products/search` для поиска каталога и не участвует в
категоризации.

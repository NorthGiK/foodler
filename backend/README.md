...
Для ранее неизвестного продукта backend запрашивает у AI-провайдера одну из
допустимых товарных категорий через `AI_LIGHT_MODEL`. Если провайдер недоступен
или возвращает некорректную категорию, сохраняется результат локальной эвристики
по тегам.
...
возвращается клиенту без преобразования; для сохранения backend использует только
проверенные поля Foodler и пропускает неподдерживаемые позиции.

...
`POST /api/receipts/get_receipt_by_raw_qr` доступен без аккаунта: он только
распознаёт фото QR-кода и не сохраняет чек или изображение. Ограничение частоты
и лимит размера файла действуют и для гостевых запросов.

...
from src.config import AI_LIGHT_MODEL

from ..analytics import get_cached_response, set_cached_response
from ..auth import get_current_user, get_current_user_optional
from ..ai_service import TaskRouter, AiServiceError, _build_user_prompt, _call_llm
...
from ..credits import get_user_credits_info, deduct_credits
...
async def run_ai(
    ...
    log_ctx = {
        ...
        "params": body.parameters.model_dump(exclude_none=True) if body.parameters else None,
    }
    ...
    # ============================================================
    # Проверка кредитов (только для LLM-уровней)
    # ============================================================
    ip = request.client.host if request and request.client else None
    is_llm_action = body.action in ("save-money", "healthy-food", "habits", "shopping-cart", "diet", "ask")

    if is_llm_action:
        credits_info = await get_user_credits_info(db=db, user=user, ip=ip)
        required = 1.0

        if credits_info.get("remaining", 0) < required and not credits_info.get("subscription"):
            logger.warning("AI request rejected: credits exhausted", extra={
                **log_ctx, "remaining": credits_info.get("remaining"),
            })
            raise HTTPException(
    ...
    logger.info("Receipts loaded", extra={
        **log_ctx, "receipt_count": len(receipts),
    })
    ...
    # ============================================================
    if body.action in ("overall-analysis", "expiring-products", "recipes", "ingredients"):
        ...
        try:
            ...
            logger.error("Failed to parse local sections", extra={
                **log_ctx, "error": str(e),
            })
        ...
        logger.info("Local AI request completed", extra={
            **log_ctx, "duration_sec": total_duration, "section_count": len(sections_parsed),
        })
    ...
    if cached:
        ...
        # ============================================================
        # Вызов TaskRouter (он сам выберет LIGHT или STRONG модель)
        # ============================================================
        ...
        logger.info("AI API response received", extra={
            **log_ctx, "duration_sec": ai_duration,
        })
        ...
        # Списание кредитов — только при реальном вызове AI (не из кэша)
        await deduct_credits(db=db, user=user, ip=ip, action=body.action)
        logger.info("Credits deducted", extra=log_ctx)
    ...
    try:
        ...
        logger.error("Failed to parse AI sections", extra={
            **log_ctx, "error": str(e),
        })
...
@post("/internal/legacy-ai-run", response_model=AiResult, include_in_schema=False)
async def legacy_run_ai(
    body: AiRequest,
    request: Request,
    ...
    # ============================================================
    # Проверка кредитов (только для LLM-действий)
    # ============================================================
    ip = request.client.host if request and request.client else None
    ...
    if cached:
        ...
        # ============================================================
        # Вызов TaskRouter (используем модульный экземпляр для тестируемости)
        # ============================================================
        parameters = body.parameters.model_dump(exclude_none=True) if body.parameters else None
        try:
            prompt: str = _build_user_prompt(body.action, parameters, context)
            raw: str = await _call_llm(AI_LIGHT_MODEL, body.action, prompt)
            sections_raw: list[dict[str, Any]] = _coerce_sections(raw)
        except AiServiceError as e:
            sections_parsed = [AiSection(type="text", title="Ошибка AI", text=f"Ошибка получения ответа от сервера: {e.message}")]
            ai_report = AiReport(
                id=uuid.uuid4().hex,
                action=body.action,
                user_id=user.id,
...
def _parse_sections(response_json: str | None) -> list[AiSection]:
    ...
    try:
        ...
        logger.warning("Failed to parse stored sections", extra={"error": str(e)})
        return []

...
class User(Base):
    ...
    # The opaque external identifier is stable, but only disclosed while the
    # account has enabled the MyTracker identity binding.
    analytics_identity_enabled: Mapped[bool] = mapped_column(nullable=False, insert_default=True)
    analytics_external_id: Mapped[str] = mapped_column(
        unique=True, nullable=False, default=lambda: uuid.uuid4().hex
    )

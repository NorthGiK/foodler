...
export function AiResultView({ report, loading, onPin, onDelete, onBack }: Props) {
    ...
    if (!report) {
        console.debug("not report is", report);
        return (
            <View style={[styles.center, { backgroundColor: theme.bg }]}>
    ...
    return (
        ...
        >
            ...
            {/* Заголовок */}
            <Text style={[styles.title, { color: theme.text }]}>{report.response.title}</Text>
